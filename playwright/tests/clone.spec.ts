...
test.describe("Duplicate and drag (Ctrl+D)", () => {
	test("Ctrl+D enters addingElements drag mode", async ({
		page,
		editor,
		pointer,
	}) => {
	...
	test("Escape while dragging duplicate cancels clone", async ({
		page,
		editor,
		pointer,
	}) => {
	...
	test("Release commits clone + selection swaps to clones", async ({
		page,
		editor,
		pointer,
	}) => {
	...
	test("Duplicate mixed component+wire while dragging", async ({
		page,
		editor,
		pointer,
	}) => {
		...
		const externalWirePathBefore = await editor
			.wires()
			.nth(1)
			.getAttribute("d");
...
test.describe("Duplicate with offset (selection toolbar button)", () => {
	...
	test("Repeated duplicate accumulates offset deterministically", async ({
		editor,
	}) => {
		await editor.addComponent("AND", 300, 300);
	...
	test("Offset duplicate of wire-only selection shifts both wire handles equally", async ({
		editor,
		pointer,
	}) => {
		await editor.addComponent("AND", 300, 300);
...
test.describe("Copy and paste", () => {
	test("Ctrl+C then Ctrl+V pastes centered at mouse", async ({
		page,
		editor,
		pointer,
	}) => {
	...
	test("Paste after original deletion still works", async ({
		page,
		editor,
	}) => {
		await editor.addComponent("AND", 300, 300);
	...
	test("Paste after undoing original placement still works", async ({
		page,
		editor,
	}) => {
		await editor.addComponent("AND", 300, 300);
	...
	test("Multiple pastes from same copy are stable", async ({
		page,
		editor,
	}) => {
	...
	test("Paste with no current selection still works", async ({
		page,
		editor,
		pointer,
	}) => {
