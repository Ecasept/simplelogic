...
test.describe("onboarding modal", () => {
	test.describe("onboarding shows/doesn't show", () => {
		test("fresh load shows onboarding preset screen", async ({ page }) => {
			...
			await expect(
				page.getByRole("heading", { name: /choose a starter circuit/i }),
			).toBeVisible();
			// Expect navigation button "Other options" visible (fresh preset screen)
			await expect(
				page.getByRole("button", { name: /other options/i }),
			).toBeVisible();
		});
	...
	test.describe("preset selection", () => {
		test("B: selecting empty preset closes modal and loads empty graph", async ({
			page,
		}) => {
			await expect(page.locator(".modal-bg")).toBeVisible();
		...
		test("selecting a non-empty preset closes modal", async ({ page }) => {
			...
			await expect(
				page.locator(".canvasWrapper .component-body >> nth=9"),
			).toBeVisible();
		});
	});

	test("manual load starts on options screen, preset does not auto-close", async ({
		page,
	}) => {
		...
		await expect(
			page.getByRole("button", { name: /paste from clipboard/i }),
		).toBeVisible();
