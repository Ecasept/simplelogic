...
test.describe("adding and dragging/moving", async () => {
	...
	test("drags and moves new wires", async ({ editor, pointer }) => {
		...
		await expect(originalHandle).toHaveAttribute("r", "6");
		await pointer.down();
	...
	test.describe("wire movement", () => {
		...
		test("multi handle flow with components", async ({ editor, pointer }) => {
			...
			const checkWires = async (expected_d: string[]) => {
				// Also assert the count to differentiate from similar helper above
				await expect(wires).toHaveCount(expected_d.length);
				for (let i = 0; i < expected_d.length; i++) {
					await expect(wires.nth(i)).toHaveAttribute("d", expected_d[i]);
				}
			};
...
test.describe("sidebars", () => {
	...
	test("remain visible on elementDown, hide while dragging, reappear after drop", async ({ editor, pointer }) => {
		...
		// Add a component and keep it selected to show the selection sidebar
		await editor.addComponent("AND", 400, 200);
		const component = editor.getComponent("AND").first();
		await expect(editor.getSidebar("selection")).toBeExpanded();

		// Press down on the component to enter elementDown state
		await pointer.downOn(component);

		// During elementDown, both sidebars should still be visible
		await expect(editor.getSidebar("tools")).toBeVisible();
		await expect(editor.getSidebar("selection")).toBeVisible();

		// Start dragging (transition to draggingElements) — sidebars should hide
		await pointer.moveTo(350, 300);
		await expect(editor.getSidebar("tools")).not.toBeVisible();
		await expect(editor.getSidebar("selection")).not.toBeVisible();

		// Drop and verify sidebars reappear in idle state
		await pointer.up();
		await expect(editor.getSidebar("tools")).toBeExpanded();
		await expect(editor.getSidebar("selection")).toBeExpanded();
	});
...
test.describe("rotation", () => {
	...
	test("rotation flow", async ({ editor }) => {
		...
		expect(Number.parseInt(movedCx)).toBe(500);
		expect(Number.parseInt(movedCy)).toBe(400);
